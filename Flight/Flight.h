...
public:
	...
	Attendant** getAttendantsList() const; // Get attendants list
	...
	bool addAttendant(const Attendant* attendant); // Add attendant
	...
	bool removeCustomerByName(const char* name); // Remove customer by name
	bool removeCustomerBySeat(const int seatRow, const int seatColumn); // Remove customer by seat
	bool resitCustomerByName(const char* name, const int seatRow, const int seatColumn); // Resit customer by name
	...
	bool changeCustomerLuggageByName(const char* name, Luggage* luggage); // Change customer luggage by name

	// General Flight details methods
	void showFlight() const; // Show flight
	char* getStatusAt(const tm time) const; // Get status of flight (not left / on air / landed)

	// Operators
	void operator=(const Flight& flight); // Operator = (there is dynamic alocation in class)
