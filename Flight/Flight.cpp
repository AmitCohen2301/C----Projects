...
Flight::Flight(const Flight& flight)
{
	...
	attendantsList = new Attendant*[flight.currentNumberOfAttendants];
	for (int attendantNum = 0; attendantNum < flight.currentNumberOfAttendants; attendantNum++)
	{
		// Copy attendant
		Attendant *curAttendant = flight.attendantsList[attendantNum];
		attendantsList[attendantNum] = new Attendant(curAttendant->getEmployeeID(), curAttendant->getName(), curAttendant->getFirstAidKnowledge(), curAttendant->getLuggage());
	}
...
Flight::Flight(Flight&& flight)
{
	...
	attendantsList = flight.attendantsList; // Move attendants list
	for (int attendantNum = 0; attendantNum < flight.currentNumberOfAttendants; attendantNum++)
	{
		attendantsList[attendantNum] = flight.attendantsList[attendantNum]; // Move attendant
		flight.attendantsList[attendantNum] = nullptr;
	}
	flight.attendantsList = nullptr;
}
...
Attendant** Flight::getAttendantsList() const // Get attendants list
{
...
Attendant* Flight::getAttendantAtPos(const int position) const // Get attendant at pos
{
	if (position > currentNumberOfAttendants || position < 1)
		return NULL;
	return attendantsList[position - 1];
}
...
bool Flight::removeAttendantAtPos(const int position) // Remove attendant at pos
{
	if (position > currentNumberOfAttendants || position < 1) // Out of list range
		return false;
	Attendant** newAttendantList = new Attendant*[this->currentNumberOfAttendants - 1];
	int indexInNewAttendentList = 0;
	for (int attendentNo = 0; attendentNo < this->currentNumberOfAttendants; attendentNo++) // Move on every attendent
	{
		if (attendentNo == position - 1) // Pos to remove
			continue;
		newAttendantList[indexInNewAttendentList] = new Attendant(attendantsList[attendentNo]->getEmployeeID(), attendantsList[attendentNo]->getName(), attendantsList[attendentNo]->getFirstAidKnowledge(), attendantsList[attendentNo]->getLuggage());
		indexInNewAttendentList++;
	}
	delete[] this->attendantsList;
	this->attendantsList = newAttendantList;
	this->currentNumberOfAttendants -= 1;
	return true;
}

bool Flight::addAttendant(const Attendant* attendant) // Add attendant
{
	if (attendant == NULL || attendant == nullptr)
		return false;
	Attendant** newAttendantList = new Attendant*[this->currentNumberOfAttendants + 1];
	if (newAttendantList == NULL)// Failed to give dynamic allocation to attendant list 
		return false;
	for (int attendantIndex = 0; attendantIndex < this->currentNumberOfAttendants; attendantIndex++) // Move on every attendent
		newAttendantList[attendantIndex] = new Attendant(attendantsList[attendantIndex]->getEmployeeID(), attendantsList[attendantIndex]->getName(), attendantsList[attendantIndex]->getFirstAidKnowledge(), attendantsList[attendantIndex]->getLuggage());
	newAttendantList[this->currentNumberOfAttendants] = new Attendant(attendant->getEmployeeID(), attendant->getName(), attendant->getFirstAidKnowledge(), attendant->getLuggage()); // Add new attendant
	delete[] this->attendantsList;
	this->attendantsList = newAttendantList;
...
bool Flight::removeAttendant(Attendant& attendant) // Remove attendant
{
	for (int attendantIndex = 0; attendantIndex < this->currentNumberOfAttendants; attendantIndex++) // Move on every attendant
		if (this->attendantsList[attendantIndex]->getEmployeeID() == attendant.getEmployeeID()) // Same attendant
			return removeAttendantAtPos(attendantIndex + 1);
	return false;
}

bool Flight::removeAllAttendants() // Remove all attendants
{
	for (int attendantsNo = 0; attendantsNo < currentNumberOfAttendants; attendantsNo++)
		delete attendantsList[attendantsNo];
...
bool Flight::removeCustomerByName(const char* name) // Remove customer by name
{
	if (plane == NULL || plane == nullptr)
		return false;
...
bool Flight::resitCustomerByName(const char* name, const int seatRow, const int seatColumn) // Resit customer by name
{
	if (seatRow < 1 || seatRow > ROWS_IN_PLANE || seatColumn < 1 || seatColumn > SEATS_PER_ROW)
		return false;
	if (plane == NULL || plane == nullptr)
...
bool Flight::changeCustomerLuggageByName(const char* name, Luggage* luggage) // Change customer luggage by name
{
	if (plane == NULL || plane == nullptr)
		return false;
...
ostream& operator<<(ostream& os, const Flight& flight) // Flight ostream
{
	...
	if (flight.currentNumberOfAttendants > 0)
	{
		...
		for (int i = 0; i < flight.currentNumberOfAttendants; i++)
			os << "\t" << "\t" << (i + 1) << ")" << *(flight.attendantsList[i]);
	}
	else
