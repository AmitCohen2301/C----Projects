...
int main()
{
	...
	do
	{
		...
		case DETAILS_MENU:
			...
			break;
		case 4:
			commandHistory.printAllCommandsHistory();
			commandHistory.addCommandToHistory("Show commands history (success)");
		default:
			break;
		}
	} while (choice != EXIT);
...
Pilot* getANewPilot()
{
	const string pilotNames[] = { "Name1", "Name2", "Name3", "Name4", "Name5", "Name6", "Name7", "Name8", "Name9", "Name10" };
...
{
	const string attendantNames[] = { "Name1", "Name2", "Name3", "Name4", "Name5", "Name6", "Name7", "Name8", "Name9", "Name10" };
...
{
	const string guardsNames[] = { "Name1", "Name2", "Name3", "Name4", "Name5", "Name6", "Name7", "Name8", "Name9", "Name10" };
	const string weaponNames[] = { "Name1", "Name2", "Name3", "Name4", "Name5", "Name6", "Name7", "Name8", "Name9", "Name10" };
...
Plane* makeNewPlane()
{
	const string planeModels[] = { "Model1", "Model2", "Model3", "Model4", "Model5", "Model6", "Model7", "Model8", "Model9", "Model10" };
...
{
	const string IATAList[] = { "IATA1", "IATA2", "IATA3", "IATA4", "IATA5", "IATA6", "IATA7", "IATA8", "IATA9", "IATA10" };
	const string countryList[] = { "Country1", "Country2", "Country3", "Country4", "Country5", "Country6", "Country7", "Country8", "Country9", "Country10" };
...
{
	Flight::getInstance()->setSourceAirport(getNewAirport());
	Flight::getInstance()->setDestinationAirport(getNewAirport());
	return Flight::getInstance();
}
...
void addPilot(Flight* flight, int designation) // main > CREW_MENU > PILOT_MENU > addPilot
{
	...
	{
		if (flight->addMainPilot(pilot))
			commandHistory.addCommandToHistory("Add new main pilot (success)");
		else
			commandHistory.addCommandToHistory("Add new main pilot (fail)");
	}
		
	else // Want to add co pilot
	{
		if (flight->addCoPilot(pilot))
			commandHistory.addCommandToHistory("Add new co pilot (success)");
		else
			commandHistory.addCommandToHistory("Add new co pilot (fail)");
	}
}
...
void pilotsMenu(Flight* flight) // main > CREW_MENU > PILOT_MENU
{
	...
	do
	{
		...
		case 2: // Remove main pilot
			if (flight->removeMainPilot())
				commandHistory.addCommandToHistory("Remove main pilot (success)");
			else
				commandHistory.addCommandToHistory("Remove main pilot (fail)");
			break;
		case 3: // Add co pilot
			addPilot(flight, 2);
			break;
		case 4: // Remove co pilot
			if (flight->removeCoPilot())
				commandHistory.addCommandToHistory("Remove co pilot (success)");
			else
				commandHistory.addCommandToHistory("Remove co pilot (fail)");
			break;
		default:
			break;
		}
...
void showAttendantsList(Flight* flight) // main > CREW_MENU > ATTENDANTS_MENU > showAttendantsList
{
	...
	LinkedList<Attendant*> allAttendants = flight->getAttendantsList();
	...
	cout << "All attendants:\n";
	for (int i = 0; i < numofAttendants; i++)
		cout << "\t" << (i+1) << ") " << *(allAttendants.getDataInIndex(i)->getData());
	commandHistory.addCommandToHistory("Show attendants list (success)");
}
...
void addAttendant(Flight* flight) // main > CREW_MENU > ATTENDANTS_MENU > addAttendant
{
	...
	if (flight->addAttendant(new Attendant(employeeID++, name, firstAidKnowledge, new Luggage(luggageWeight, luggageVolume))))
		commandHistory.addCommandToHistory("Add attendant (success)");
	else
		commandHistory.addCommandToHistory("Add attendant (fail)");
}
...
void removeAttendant(Flight* flight)// main > CREW_MENU > ATTENDANTS_MENU > removeAttendant
{
	...
	if (flight->removeAttendantAtPos(attendantOffset))
		commandHistory.addCommandToHistory("Remove attendant (success)");
	else
		commandHistory.addCommandToHistory("Remove attendant (fail)");
}
...
void attendantMenu(Flight* flight) // main > CREW_MENU > ATTENDANTS_MENU
{
	...
	do
	{
		...
		case 3:
			...
			break;
		case 4:
			if(flight->removeAllAttendants())
				commandHistory.addCommandToHistory("Remove all attendants (success)");
			else
				commandHistory.addCommandToHistory("Remove all attendants (fail)");
			break;
		default:
			break;
		}
...
void addSecurityGuard(Flight* flight, int designation) // main > CREW_MENU > SECURITY_GUARDS_MENU > addGuard
{
	...
	{
		if (flight->addSecurityGuard1(securityGuard))
			commandHistory.addCommandToHistory("Add security guard 1 (success)");
		else
			commandHistory.addCommandToHistory("Add security guard 1 (fail)");
	}
	else // Want to add security guard1
	{
		if (flight->addSecurityGuard2(securityGuard))
			commandHistory.addCommandToHistory("Add security guard 2 (success)");
		else
			commandHistory.addCommandToHistory("Add security guard 2 (fail)");
	}
}
...
void guardsMenu(Flight* flight) // main > CREW_MENU > SECURITY_GUARDS_MENU
{
	...
	do
	{
		...
		case 1:
			...
			break;
		case 2:
			if (flight->removeSecurityGuard1())
				commandHistory.addCommandToHistory("Remove security guard 1 (success)");
			else
				commandHistory.addCommandToHistory("Remove security guard 1 (fail)");
			break;
		case 3:
			addSecurityGuard(flight, 2);
			break;
		case 4:
			if (flight->removeSecurityGuard2())
				commandHistory.addCommandToHistory("Remove security guard 2 (success)");
			else
				commandHistory.addCommandToHistory("Remove security guard 2 (fail)");
			break;
		default:
			break;
		}
...
void addCustomerNoSit(Flight* flight) // main > CUSTOMERS_MENU > ADD_CUSTOMERS_MENU > add customer without seat
{
	...
	if (*flight += *(new Customer(ticketNum, name, new Luggage(luggageWeight, luggageVolume))))
		commandHistory.addCommandToHistory("Add customer with no sit (success)");
	else
		commandHistory.addCommandToHistory("Add customer with no sit (fail)");
}
...
void addCustomerWithSit(Flight* flight) // main > CUSTOMERS_MENU > ADD_CUSTOMERS_MENU > add customer with seat
{
	...
	if(flight->addCustomer(new Customer(ticketNum, name, new Luggage(luggageWeight, luggageVolume)), seatRow, seatColumn))
		commandHistory.addCommandToHistory("Add customer with sit (success)");
	else
		commandHistory.addCommandToHistory("Add customer with sit (fail)");
}
...
void removeCustomerByName(Flight* flight) // main > CUSTOMERS_MENU > REMOVE_CUSTOMERS_MENU > REMOVE_CUSTOMER_BY_NAME
{
	...
	if (flight->removeCustomerByName(name))
		commandHistory.addCommandToHistory("Remove customer by name (success)");
	else
		commandHistory.addCommandToHistory("Remove customer by name (fail)");
}
...
void removeCustomerBySeat(Flight* flight) // main > CUSTOMERS_MENU > REMOVE_CUSTOMERS_MENU > REMOVE_CUSTOMER_BY_SEAT
{
	...
	if (flight->removeCustomerBySeat(seatRow, seatColumn))
		commandHistory.addCommandToHistory("Remove customer by seat (success)");
	else
		commandHistory.addCommandToHistory("Remove customer by seat (fail)");
}
...
void resitCustomerByName(Flight* flight) // main > CUSTOMERS_MENU > RESIT_CUSTOMERS_MENU > RESIT_CUSTOMER_BY_NAME
{
	...
	if (flight->resitCustomerByName(name, newSeatRow, newSeatColumn))
		commandHistory.addCommandToHistory("Resit customer by name (success)");
	else
		commandHistory.addCommandToHistory("Resit customer by name (fail)");
}
...
void resitCustomerBySeat(Flight* flight) // main > CUSTOMERS_MENU > RESIT_CUSTOMERS_MENU > RESIT_CUSTOMER_BY_SEAT
{
	...
	if(flight->resitCustomerBySeat(oldSeatRow, oldSeatColumn, newSeatRow, newSeatColumn))
		commandHistory.addCommandToHistory("Resit customer by seat (success)");
	else
		commandHistory.addCommandToHistory("Resit customer by seat (fail)");
}
...
void changeCustomerLuggage(Flight* flight) // main > CUSTOMERS_MENU > CHANGE_CUSTOMER_LUGGAGE
{
	...
	if (flight->changeCustomerLuggageByName(name, new Luggage(luggageWeight, luggageVolume)))
		commandHistory.addCommandToHistory("Change customer luggage (success)");
	else
		commandHistory.addCommandToHistory("Change customer luggage (fail)");
}
...
void setSourceAirport(Flight* flight) // main > DETAILS_MENU >> AIRPORT > set source
{
	...
	cin >> countryName;
	if (flight->setSourceAirport(new Airport(IATA, countryName)))
		commandHistory.addCommandToHistory("Set source airport (success)");
	else
		commandHistory.addCommandToHistory("Set source airport (fail)");
}
...
void setDestinationAirport(Flight* flight) // main > DETAILS_MENU >> AIRPORT > set Destination
{
	...
	cout << "Enter the airport's country name: ";
	cin >> countryName;
	if (flight->setDestinationAirport(new Airport(IATA, countryName)))
		commandHistory.addCommandToHistory("Set destination airport (success)");
	else
		commandHistory.addCommandToHistory("Set destination airport (fail)");
}
...
void setTime(Flight* flight, int designation)
{
	...
	{
		if (flight->setTakeoffTime(&time))
			commandHistory.addCommandToHistory("Set takeoff time (success)");
		else
			commandHistory.addCommandToHistory("Set takeoff time (fail)");
	}
	else // Want to set landing time
	{
		if (flight->setLandingTime(&time))
			commandHistory.addCommandToHistory("Set landing time (success)");
		else
			commandHistory.addCommandToHistory("Set landing time (fail)");
	}
}
...
void getStatusByTime(Flight* flight) // main > DETAILS_MENU > get status by time
{
	...
	commandHistory.addCommandToHistory("Get status by time (success)");
}
...
void flightDetailsMenu(Flight* flight) // main > DETAILS_MENU
{
	...
	do
	{
		...
		case 1:
			cout << *flight;
			commandHistory.addCommandToHistory("Print flight (success)");
			break;
		case 2:
			flight->getPlane()->showSeats();
			commandHistory.addCommandToHistory("Print seats (success)");
			break;
		case 3:
